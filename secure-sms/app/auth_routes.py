"""Routes for user registration, login, 2FA and logout."""
...
def register():
    """Handle user registration via a simple form."""
    form = RegisterForm()
...
def login():
    """Authenticate the user and start the OTP process."""
...
def otp():
    """Handle OTP verification for the second factor."""
    ...
    if form.validate_on_submit():
        ...
        if real and str(code) == str(real) and datetime.utcnow().timestamp() <= exp:
            ...
            current_app.audit_logger.warning(
                f"2FA failure for preauth user id {session.get('preauth_user_id')}"
            )
